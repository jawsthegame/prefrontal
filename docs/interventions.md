...
| `winddown_check` | From `winddown_start_hour` (evening) to `winddown_end_hour`, a "start winding down for bed" reminder every `winddown_reask_minutes` until you confirm. Off unless `winddown_enabled` — a bedtime is personal. An evening nudge, so it bypasses the shared daytime quiet-hours window by default (`winddown_bypass_quiet_hours`) and self-bounds with its own end hour instead. One-tap Winding down / Snooze action buttons on the push. | evening run-up to bed, until you confirm you're winding down |
| `movement_check` | From `movement_start_hour`, a "time to move" nudge for a few minutes of movement / stretching, re-asked every `movement_reask_minutes` until you confirm. The daily-movement *floor* — a tiny, always-doable minimum that survives the worst day so the habit never hits zero. Off unless `movement_enabled` — an exercise cadence is personal; defaults to the morning (07:00), anchored to the first coffee, an existing daily pause you habit-stack onto. One-tap Stretched / Snooze action buttons on the push. | from your movement hour, until you confirm you've moved |
| `self_care_review` | An opt-in evening recap that reads the day's self-care clicks back as a timeline and names the gaps a raw count hides — a late first glass of water, a long stretch between bio breaks, a quota that finished short — plus what went well. From `self_care_review_hour` (default 21:00), once, and only when there's a gap worth naming (a clean day stays silent). Off unless `self_care_review_enabled` — a daily recap is personal. Bypasses the daytime quiet-hours window by default (`self_care_review_bypass_quiet_hours`) so an end-of-day recap actually lands; it can't nag (once/day, evening-only). A plain informational push (no buttons); also on `prefrontal self-care review` / `GET /self-care/review`. | evening, once, when the day's clicks show a gap |
