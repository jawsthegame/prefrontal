...
| Vacation mode | `prefrontal/vacation.py` | ✅ Ease off the nudges while away — a *receptivity profile*, not a kill switch: holds every discretionary cue (as quiet hours does) while letting `critical` calendar obligations and on-demand surfaces (panic, emotion) through. **Cue-based detect-and-confirm**: after ~2 nights away the trip tracker raises a one-tap `🏝️ Ease off` suggestion (once per absence, no re-nag); returning home **auto-resumes** it (so a forgotten toggle can't leave you muted). Manual `prefrontal vacation on/off/status` + `GET`/`POST /vacation` is the escape hatch for staycations. See [`docs/design/vacation-mode.md`](docs/design/vacation-mode.md) |
