...
class PlansRepo(Repo):
    ...
    def add_implementation_intention(
        ...
        *,
        ...
        A plan is a *cue* (a curated place, a local time-of-day band, or both) paired
        with a tiny *pre-decided action*. At least one of ``cue_place``/``cue_window``
        should be set for the plan to ever fire — a plan with neither is stored (so a
        half-captured plan isn't lost) but simply never matches until a cue is added.

        Args:
            cue_text: The trigger in the user's own words ("when I sit at my desk
                after lunch"), shown back verbatim.
        ...
        cur = self.conn.execute(
            ...
            "(user_id, cue_text, cue_place, cue_window, action_text, todo_id) "
            "VALUES (?, ?, ?, ?, ?, ?)",
            (self._uid(), cue_text, cue_place, cue_window, action_text, todo_id),
        )
        self.conn.commit()
