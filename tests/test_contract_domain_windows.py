"""Contract guard for the ``/schedule/domain-windows`` shape.

The ``DomainWindows`` / ``DomainWindow`` contract is **hand-mirrored** in three
...
server's **structural** schema (field names, types, patterns, defaults, domain
...
Update the snapshot **on purpose** (after moving the web + iOS mirrors) with::
    ...
    UPDATE_CONTRACT=1 uv run pytest tests/test_contract_domain_windows.py
"""
...
from prefrontal.webhooks.schemas import DomainWindows
from tests.conftest import scoped_default

CONTRACTS = Path(__file__).parent / "contracts"
SCHEMA_SNAPSHOT = CONTRACTS / "domain_windows.schema.json"
EXAMPLE = CONTRACTS / "domain_windows.example.json"

#: The models whose structural schema both clients mirror.
CONTRACT_MODELS = ("DomainWindows", "DomainWindow")


def _strip_prose(node: Any) -> Any:
    """Recursively drop non-structural keys (``description``/``title``).

    Leaves everything a client mirror actually depends on — property names,
...
def test_domain_windows_schema_matches_snapshot() -> None:
    live = _live_schema()
    ...
    assert live == committed, (
        "The /schedule/domain-windows schema drifted from the committed contract.\n"
...
def test_example_fixture_is_valid_and_canonical() -> None:
    ...
    parsed = DomainWindows(**payload)  # raises if the example no longer validates
    # The example is the canonical full reference: every life domain, a mix of
    # configured windows and a cleared (inherited) one, so every branch is exercised.
    assert set(parsed.domains) == set(FOCUS_DOMAINS)
    assert any(not d.configured for d in parsed.domains.values()), "cover a cleared domain"
    assert any(d.configured for d in parsed.domains.values()), "cover a configured domain"
