...
// MARK: - Brain-dump (roadmap M1 "capture at the speed of thought")

/// The framework-free result of an on-device parse (`BrainDumpParser`), kept in
/// Models so the networking layer (compiled into the widget too) can reference it
/// without importing the app-only, Foundation-Models-gated parser. `wireActions`
